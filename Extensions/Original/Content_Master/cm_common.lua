...
-- отсекает в УРЛ префиксы, добавляемые WebWarper и OperaTurbo, преобразует из punycode в национальные символы
function _CM_NORMALIZE_URL(url)
	...
	return url,	((url:match('^https?://[^:/?]*') or '') .. '/'), (url:match('^https?://([^:/?]*)') or '')	-- возвращаем также имя хоста и домена
end
