...
-- if not loadstring then loadstring = load end	-- loadstring в Lua 5.2 заменена на load
-- if not unpack then unpack = table.unpack end	-- unpack в Lua 5.2 заменена на table.unpack
...
function BeforeRequestHeaderSend()
	...
	-- elseif _CM_CURRENT_REQUEST_DATA.URL:match('^https?://local%.cm/editor/') then	-- рудимент???
...
-- clck1 = clck1 - os.clock()
	...
	if _CM_CURRENT_REQUEST_DATA.ContentTypeByHeader=='html' then
		...
		if CSP then
			if _CM_CURRENT_REQUEST_DATA.InteractiveModeOn then CSP = CSP:gsub("(script%-src[^;]-)%s+'nonce%-.-'", '%1', 1) end	-- отменяем запрет на инлайн-скрипты для успешного включения интерактивного режима
			local n, n1
